...
static u_char *
ngx_stream_lua_socket_udp_log_error(ngx_log_t *log, u_char *buf, size_t len)
{
    u_char                       *p;
...
ngx_stream_lua_socket_udp_setpeername(lua_State *L)
{
    ...
    uc->log.data = u;
    uc->log.handler = ngx_stream_lua_socket_udp_log_error;
...
ngx_stream_lua_udp_sendmsg(ngx_connection_t *c, ngx_iovec_t *vec)
{
    ...
    u_char         msg_control[CMSG_SPACE(sizeof(struct in_pktinfo))] = {0};
#endif

#if (NGX_HAVE_INET6 && NGX_HAVE_IPV6_RECVPKTINFO)
    u_char         msg_control6[CMSG_SPACE(sizeof(struct in6_pktinfo))] = {0};
#endif
